...
export class AuthService {
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    if (error.status === 0) {
      // Network error
      errorMessage = 'Cannot connect to server. Please check your network connection or try again later.';
      ...
      error.status === 500 &&
      error.error &&
      (typeof error.error === 'string' && error.error.toLowerCase().includes('bad credentials') ||
        error.error.message && error.error.message.toLowerCase().includes('bad credentials'))
    ) {
      // Backend returns 500 with 'Bad credentials' message
      errorMessage = 'Invalid username or password.';
